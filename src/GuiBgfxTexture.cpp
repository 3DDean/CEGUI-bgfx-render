...
namespace CEGUI 
{
	...
	void GuiBgfxTexture::loadFromFile(const String & filename, const String & resourceGroup)
	{
		...
		System::getSingleton().getResourceProvider()->loadRawDataContainer(filename, fileContainer, resourceGroup);
		void* dataPtr = fileContainer.getDataPtr();
		int size = fileContainer.getSize();

		bimg::ImageContainer *imageContainer = bimg::imageParse(allocator, dataPtr, size);
		...
		System::getSingleton().getResourceProvider()->unloadRawDataContainer(fileContainer);
	}
	...
	void GuiBgfxTexture::setAllocator(bx::AllocatorI * alloc)
	{
		allocator = alloc;
	}
